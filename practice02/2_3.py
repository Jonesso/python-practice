...
def f23(table):
    sort_table(table)
    table = delete_duplicates(table)
    rename_elements(table)
    for line in table:
        print(line)
    return table
...
def sort_table(table):
    table.sort(key=key_func)


# функция приведения элементов к нужному формату
def rename_elements(table):
    for line in table:
        for cell in range(len(line)):
            ...
            if match:
                line[cell] = match[0][:2] + " (" + match[0][2:5] + ") " + match[0][5:8] + "-" + match[0][8:10] + "-" + \
                             match[0][10:]
